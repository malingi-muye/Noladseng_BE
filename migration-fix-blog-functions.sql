-- Migration: Fix blog post functions - Drop old versions and create corrected ones
-- Run this SQL in your Supabase SQL Editor to fix the function conflicts

-- Drop the old functions with INTEGER[] parameters that are causing conflicts
...
-- Create the corrected blog post management functions with TEXT[] parameters
...
BEGIN
    ...
    -- Convert post_data tags JSONB to TEXT[] if provided
    IF post_data->'tags' IS NOT NULL THEN
        SELECT ARRAY(SELECT jsonb_array_elements_text(post_data->'tags')) INTO post_tags_array;
    END IF;
    ...
    INSERT INTO public.blog_posts (
        ...
        COALESCE(tags_array, post_tags_array),
...
BEGIN
    ...
    -- Convert post_updates tags JSONB to TEXT[] if provided
    IF post_updates->'tags' IS NOT NULL THEN
        SELECT ARRAY(SELECT jsonb_array_elements_text(post_updates->'tags')) INTO update_tags_array;
    END IF;
    ...
    UPDATE public.blog_posts SET
        ...
        tags = COALESCE(tags_array, update_tags_array, tags),

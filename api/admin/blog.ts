...
// Create CRUD handlers for blog posts
const {
  getAll,
  getById,
  create,
...
const getAllBlogPosts = async (req: VercelRequest, res: VercelResponse) => {
  const { search, category, featured, status, page = '1', limit = '10' } = req.query;
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      error: `Error fetching blog posts: ${error.message}`
    });
  }
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log(`[Admin Blog ${requestId}] ${req.method} ${req.url}`, {
    params: req.query,
    body: req.body,
  });
  ...
  try {
    ...
    console.error(`[Admin Blog ${requestId}] Error:`, error);
    return res.status(500).json({
      success: false,

...
const getAllContacts = async (req: VercelRequest, res: VercelResponse) => {
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      error: `Error fetching contact messages: ${error.message}`
    });
  }
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  console.log(`[Admin Contacts ${requestId}] ${req.method} ${req.url}`, {
    params: req.query,
    body: req.body,
  });
  ...
  try {
    ...
    console.error(`[Admin Contacts ${requestId}] Error:`, error);
    return res.status(500).json({
      success: false,
